...
const MyProjects = () => {
    return (
        <section id="my_projects">
            <div className="sec-bg-color px-5 pt-12 pb-20 ">
                <h1 className="text-center text-2xl font-semibold uppercase">My projects</h1>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {menu.map((item) => (
                        <div key={item.id} className="bg-[#0B0F12] p-5 rounded-lg text-center mx-9 mt-3 border-[2px] border-[#00ffe0] drop-shadow-[0px_4px_7px_#00ffe0]">
                            <img src={item.img} alt={item.title} className="h-32 mx-auto mb-4" />
                            <h2 className="text-xl font-bold mb-2">{item.title}</h2>
                            <p className="mb-4 text-center">{item.description}</p>
                            <div className="flex items-center justify-center p-2 bg-[#00ffe0] text-black text-lg font-semibold rounded-lg hover:scale-110 transition mx-6">
